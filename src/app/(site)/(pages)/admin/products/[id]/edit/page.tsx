...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => { loadProduct(); loadCategories(); }, [productId]);

  const loadProduct = async () => {
    try {
      setLoading(true);
      const data = await getProductById(productId);
      if (!data) { toast.error("Product not found"); router.push("/admin/products"); return; }
    ...
    } catch (error) { toast.error("Failed to load product"); } finally { setLoading(false); }
  };

  const loadCategories = async () => {
    try { const data = await getAllCategories(); setCategories(data); } catch (error) { toast.error("Failed to load categories"); }
  };
  ...
  const openNewVariantForm = () => { setCurrentVariant({ id: "", sku: generateVariantSKU(productSku, {}), price: 0, stock: 0, color: availableColors[0] || null, size: availableSizes[0] || null, storage: availableStorage[0] || null, images: [], weight: weight ? parseFloat(weight) : null, isDefault: variants.length === 0, }); setIsNewVariant(true); setShowVariantForm(true); };
  const openEditVariantForm = (variant: Variant) => { setCurrentVariant(variant); setIsNewVariant(false); setShowVariantForm(true); };
  
  const saveVariant = async () => {
    if (!currentVariant) return;
    if (!currentVariant.price || currentVariant.price <= 0) { toast.error("Please enter a valid price"); return; }
    if (currentVariant.stock < 0) { toast.error("Please enter a valid stock quantity"); return; }
    try {
      setSaving(true);
      if (isNewVariant) {
        await addVariant({ productId, sku: currentVariant.sku, price: currentVariant.price, stock: currentVariant.stock, color: currentVariant.color || undefined, size: currentVariant.size || undefined, storage: currentVariant.storage || undefined, images: currentVariant.images, weight: currentVariant.weight || undefined, isDefault: currentVariant.isDefault, });
        toast.success("Variant added");
      } else {
        await updateVariant(currentVariant.id, { sku: currentVariant.sku, price: currentVariant.price, stock: currentVariant.stock, color: currentVariant.color || undefined, size: currentVariant.size || undefined, storage: currentVariant.storage || undefined, images: currentVariant.images, weight: currentVariant.weight || undefined, isDefault: currentVariant.isDefault, });
        toast.success("Variant updated");
      }
      await loadProduct(); setShowVariantForm(false);
    } catch (error) { console.error("Error saving variant:", error); toast.error("Failed to save variant"); } finally { setSaving(false); }
  };

  const handleDeleteVariant = async (variantId: string) => {
    if (!confirm("Are you sure?")) return;
    if (variants.length === 1) { toast.error("Cannot delete last variant"); return; }
    try { setSaving(true); await deleteVariantAction(variantId); toast.success("Variant deleted"); await loadProduct(); } catch (error) { toast.error("Failed to delete variant"); } finally { setSaving(false); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !description || !imageUrl || !categoryId) { toast.error("Required fields missing"); return; }
    setSaving(true);
    try {
  ...
  // Helper styles matching add-product/page.tsx
  ...
  if (loading || !product) return <div className="min-h-screen bg-meta flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue"></div></div>;
    ...
    <div className="min-h-screen bg-meta p-6 font-euclid-circular-a">
      <div className="max-w-[1600px] mx-auto">
        <header className="mb-8 flex justify-between items-end">
          <div>
            <div className="text-custom-sm font-bold text-dark-5 mb-1 uppercase tracking-widest">Editor</div>
            <h1 className="text-heading-4 font-bold text-dark">Edit Product: {title}</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className={cardStyle}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea value={description} onChange={(e) => setDescription(e.target.value)} className={inputStyle} rows={6} required />
                </div>
                <div>
                  <label className={labelStyle}>Short Description</label>
                  <input type="text" value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} className={inputStyle} />
                </div>
              </div>
            </section>
            ...
            <section className={cardStyle}>
              <div className="flex justify-between items-center mb-6">
                ...
                <button type="button" onClick={openNewVariantForm} className="px-4 py-2 bg-green-light-6 text-green-dark border border-green-light-4 rounded-lg font-bold text-2xs uppercase tracking-wider hover:bg-green-light-5 transition-all">
                  + Add Variant
                </button>
              </div>
                    ...
                    {variants.map((variant) => (
                      <tr key={variant.id} className="hover:bg-gray-1/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right">
                          <button type="button" onClick={() => openEditVariantForm(variant)} className="text-blue hover:text-blue-dark font-bold text-2xs uppercase mr-3">Edit</button>
                          <button type="button" onClick={() => handleDeleteVariant(variant.id)} className="text-red hover:text-red-dark font-bold text-2xs uppercase">Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="space-y-8">
            ...
            <div className={cardStyle}>
              <button type="submit" disabled={saving} className="w-full py-4 bg-blue text-white rounded-xl font-bold shadow-2 hover:bg-blue-dark transition-all disabled:opacity-50 mb-3">
            ...
            {/* Options Management */}
            <section className={cardStyle}>
              <h2 className="text-custom-lg font-bold text-dark mb-4">Available Options</h2>
              <p className="text-xs text-body mb-4">These options save to the product and populate the variant generator.</p>
              
              {/* Colors */}
              <div className="mb-4">
                <label className={labelStyle}>Colors</label>
                <div className="flex gap-2 mb-2">
                  <input type="text" value={colorInput} onChange={(e) => setColorInput(e.target.value)} className={inputStyle} />
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2 mb-2">
                  <input type="text" value={sizeInput} onChange={(e) => setSizeInput(e.target.value)} className={inputStyle} />
               ...
               <div className="mb-4">
                ...
                <div className="flex gap-2 mb-2">
                  <input type="text" value={storageInput} onChange={(e) => setStorageInput(e.target.value)} className={inputStyle} />
